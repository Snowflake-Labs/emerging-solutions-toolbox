...
def render_prereqs():
    ...
    try:
        current_region = pd.DataFrame(
            session.sql("SELECT CURRENT_REGION()").collect()
        ).iloc[0, 0]
    except:
        st.rerun()
...
def render_create_ev():
    ...
    st.caption(
        ":red[⚠︎ NOTE:  The current role should be either ACCOUNTADMIN or have been granted the CREATE EXTERNAL VOLUME privilege.]"
    )
    st.write("")
    ...
    if st.session_state.txt_cloud == "AWS":
        ...
        txt_ev_name = st.text_input("External Volume Name:", key="txt_ev_name")

        # storage provider
        aws_storage_provider = "S3GOV" if "gov" in current_region.lower() else "S3"
        txt_ev_storage_provider = st.text_input(
            "Storage Provider:", aws_storage_provider, disabled=True
        )

        # AWS role ARN
        txt_aws_role_arn = st.text_input(
            "AWS Role ARN:",
        ...
        # AWS base URL
        txt_base_url = st.text_input(
            "Base URL:", help="i.e.: s3://<my_bucket>/", key="txt_base_url"
        )

        # AWS storage access point ARN (if applicable)
        aws_storage_access_point_arn_field = ""
        if txt_base_url.endswith("-s3alias"):
            txt_aws_storage_access_point_arn = st.text_input(
                "AWS Storage Access Point ARN:",
            ...
            if txt_aws_storage_access_point_arn == "":
                st.error(
                    f"The AWS base URL is aliased, which requires an AWS Storage Access Point ARN. Please provide.",
                    icon="🚨",
                )
            aws_storage_access_point_arn_field = (
                f"STORAGE_AWS_ACCESS_POINT_ARN = '{txt_aws_storage_access_point_arn}'"
            )

        # AWS external ID
        txt_aws_external_id = st.text_input(
            "AWS External ID:",
        ...
        aws_external_id_field = (
            ""
            if txt_aws_external_id == ""
            else f"STORAGE_AWS_EXTERNAL_ID = '{txt_aws_external_id}'"
        )

        # encryption (optional)
        encryption_type_list = ["AWS_SSE_S3", "AWS_SSE_KMS", "NONE"]
        sb_encryption_type = st.selectbox(
            "Encryption Type:", encryption_type_list, index=2, key="sb_encryption_type"
        )
        kms_key_id_field = ""
        if sb_encryption_type == "AWS_SSE_KMS":
            txt_kms_key_id = st.text_input(
                "KMS Key ID:",
            ...
            if txt_kms_key_id == "":
                st.error(
                    f"{sb_encryption_type} requires a KMS Key ID. Please provide.",
                    icon="🚨",
                )
            else:
        ...
        encryption_type_field = (
            ""
            if sb_encryption_type == "NONE"
            else f"ENCRYPTION=(TYPE = '{sb_encryption_type}'{kms_key_id_field})"
        )

        # use privatelink (optional)
        use_privatelink_endpoint_list = ["TRUE", "FALSE"]
        sb_use_privatelink_endpoint = st.selectbox(
        ...
        use_privatelink_endpoint_field = (
            ""
            if sb_use_privatelink_endpoint == "FALSE"
            else f"USE_PRIVATELINK_ENDPOINT = TRUE"
        )
        ...
        if (
            txt_base_url.endswith("-s3alias") and txt_aws_storage_access_point_arn == ""
        ) or (sb_encryption_type == "AWS_SSE_KMS" and kms_key_id_field == ""):
            flag_disable_btn_create_ev = True
        ...
        with col3:
            st.write("")
            st.write("")
            btn_create_ev = st.button(
                "Create", type="primary", disabled=flag_disable_btn_create_ev
            )
        ...
        if flag_create_ev:
            with st.spinner("Updating..."):
                session.sql(
                    f"""CREATE OR REPLACE EXTERNAL VOLUME {txt_ev_name}
                                   STORAGE_LOCATIONS =
                                      (
                                         (
                                            NAME = '{txt_ev_name}'
    ...
    if st.session_state.txt_cloud == "AZURE":
        ...
        txt_ev_storage_provider = st.text_input(
            "Storage Provider:", "AZURE", disabled=True
        )

        # Azure tenant ID
        txt_azure_tenant_id = st.text_input(
            "Azure Tenant ID:",
        ...
        # Azure base URL
        txt_base_url = st.text_input(
            "Base URL:", help="i.e.: azure://...", key="txt_base_url"
        )
    ...
    if st.session_state.txt_cloud == "GCP":
        ...
        txt_ev_storage_provider = st.text_input(
            "Storage Provider:", "GCS", disabled=True
        )
        st.session_state.ev_storage_provider = txt_ev_storage_provider

        txt_base_url = st.text_input(
            "Base URL:", help="i.e.: gcs://<bucket>/<path>/", key="txt_base_url"
        )

        # encryption (optional)
        encryption_type_list = ["GCS_SSE_KMS", "NONE"]
        sb_encryption_type = st.selectbox(
            "Encryption Type:", encryption_type_list, index=1, key="sb_encryption_type"
        )
        kms_key_id_field = ""
...
def input_callback(session_key, input_key):
    ...
    if input_key.lower() == "txt_enter_query_id":
        if st.session_state.selected_query_id == "" or (
            st.session_state.enter_query_id == st.session_state.selected_query_id
        ):
...
def run_nonsis_cmd(cmd, generate_results_flag):
    df_cmd_results = pd.DataFrame(
        "Dataframe not generated", index=[0], columns=["Status"]
    )
    ...
    if generate_results_flag:
        cmd_hash = pd.DataFrame(
            session.sql(
                f"""SELECT "SIS_CMD_MGR_<MY_ROLE>".UTIL.TABLE_UUID($${cmd}$$)"""
            ).collect()
        ).iloc[
            0, 0
        ]  # hash of the command

    start_timestamp = pd.DataFrame(session.sql(f"SELECT SYSDATE()").collect()).iloc[
        0, 0
    ]

    session.sql(
        f"""INSERT INTO "SIS_CMD_MGR_<MY_ROLE>".CMD_MGR.COMMANDS(app_name, cmd, cmd_hash, status, start_timestamp, completed_timestamp, generate_results_table, results_table, notes)
                    SELECT 
                        'CONVERT2ICEBERG'
                        ,$${cmd}$$
                        ,'{cmd_hash}'
    ...
    while True:
        df_cmd_status = pd.DataFrame(
            session.sql(
                f"""SELECT status, results_table, notes FROM "SIS_CMD_MGR_<MY_ROLE>".CMD_MGR.COMMANDS WHERE app_name = 'CONVERT2ICEBERG' AND run_id = '{run_id}'"""
            ).collect()
        )
        ...
        if (
            (status != "PROCESSING")
            or ((cmd_end_time - cmd_start_time) >= timeout)
            or status.lower() == "error"
        ):
            break
    ...
    if generate_results_flag and status.lower() == "complete":
        df_cmd_results = pd.DataFrame(
            session.sql(
                f"""SELECT * FROM "SIS_CMD_MGR_<MY_ROLE>".RESULTS."{results_table}";"""
            ).collect()
        )
...
def get_query_id_from_qh():
    if st.session_state["query_history_id"].selection["rows"]:
        ...
        df_selected_query_idx = st.session_state["query_history_id"].selection["rows"][
            0
        ]

        # set selected query ID
        st.session_state.selected_query_id = (
            st.session_state.df_query_history_range.iloc[df_selected_query_idx, 0]
        )
        st.session_state.disable_step_2 = False


def render_query_stats(query_id):
    df_query_history_id = pd.DataFrame(
        session.sql(
            f"""SELECT
                                                            --query details
                                                            QUERY_ID
                                                        ...
                                                        WHERE
                                                            ...
                                                            LOWER(EXECUTION_STATUS) = 'success';"""
        ).collect()
    )
    ...
    if not df_query_history_id.empty:
        ...
        df_query_details = (
            df_query_history_id[["QUERY_ID", "QUERY_TYPE", "QUERY_TEXT"]]
            .copy()
            .transpose()
        )

        # execution details
        df_execution_details = (
            df_query_history_id[
                [
                    "WAREHOUSE_NAME",
                    "WAREHOUSE_SIZE",
        ...
        # execution breakdown
        df_execution_breakdown_details = (
            df_query_history_id[
                [
                    "COMPILATION_TIME",
                    "EXECUTION_TIME",
        ...
        with st.container(border=True, key=query_id):
            ...
            st.write("")
            st.markdown(
                df_query_details.style.set_table_styles(
            ...
            st.write("")
            st.markdown(
                df_execution_details.style.set_table_styles(
                    [{"selector": "th", "props": [("font-size", "12px")]}]
                )
                .set_properties(
                    **{
            ...
            with col2:
                if not df_execution_breakdown_details.empty:
                    ...
                    ex_bd_pie.update_layout(
                        ...
                        legend=dict(yanchor="bottom", y=-0.35, xanchor="left", x=0.8),
                    )
                    ex_bd_pie.update_traces(
                        textposition="inside", textinfo="percent+label"
                    )
                    st.plotly_chart(
                        ex_bd_pie, theme="streamlit", use_container_width=False
                    )
            ...
            st.write("")
            st.markdown(
                df_bytes_details.style.set_table_styles(
                    [{"selector": "th", "props": [("font-size", "12px")]}]
                )
                .set_properties(
                    **{
        ...
        st.markdown(
            "<h3 style='text-align: center; color: black;'>Stats Currently Unavailable</h3>",
            unsafe_allow_html=True,
        )
...
def query_stats_compare(org_query_id, converted_query_id):
    ...
    with col1:
        st.markdown(
            "<h2 style='text-align: center; color: black;'>Original Query</h2>",
            unsafe_allow_html=True,
        )
    ...
    with col2:
        st.markdown(
            "<h2 style='text-align: center; color: black;'>Query w/updated Iceberg Tables</h2>",
            unsafe_allow_html=True,
        )
...
def render_create_iceberg_table(ev_name, table):
    ...
    with col2:
        st.text_input("", ev_name, key="selected_external_volume", disabled=True)
    ...
    with col1:
        ...
        st.write("**Table Name:**")
    with col2:
        st.text_input("", table, key="selected_table", disabled=True)
    ...
    df_table_cols = pd.DataFrame(
        session.sql(
            f"""SELECT "name", "type" FROM TABLE(RESULT_SCAN(LAST_QUERY_ID()))"""
        ).collect()
    )
  ...
  FROM {table}"""
    ...
    # beautify DDL
    create_iceberg_ddl_pretty = sqlparse.format(
        create_iceberg_ddl, reindent=False, keyword_case="upper"
    )
    ...
    with col3:
        btn_create_it = st.button(
            "Create", type="primary", disabled=disable_create_it_btn
        )
...
def render_choose_table_wizard_view():
    ...
    st.markdown(
        "<h2 style='text-align: center; color: black;'>Convert Existing Table</h2>",
        unsafe_allow_html=True,
    )
    ...
    st.write("")
    st.write("")
    step_cols = st.columns([0.65, 0.55, 0.55, 0.55, 0.5])
    step_cols[1].button(
        "STEP 1",
    ...
    step_cols[2].button(
        ...
        args=["Jump", 2],
        type=btn_tbl_type,
        disabled=st.session_state.disable_step_2,
    )
    step_cols[3].button(
        "STEP 3",
        on_click=set_form_step,
        args=["Jump", 3],
        type=btn_ib_tbl_type,
        disabled=st.session_state.disable_step_3,
    )
    st.write("")
    st.write("")
    ...
    if st.session_state.current_step == 1:
        ...
        st.caption(
            ":red[⚠︎ NOTE:  If an External Volume does not exist, use the app's **CREATE EXTERNAL VOLUME** tool to create one.]"
        )
    ...
    if st.session_state.current_step == 2:
        ...
        if not dbs.empty:
            ...
            schemas = pd.DataFrame(
                session.sql(f"""SHOW SCHEMAS IN DATABASE {sb_db}""").collect()
            )

        if not schemas.empty:
            sb_schema = st.selectbox(
                "Select Schema:",
                schemas["name"],
            ...
            tables = pd.DataFrame(
                session.sql(
                    f"""SELECT * FROM TABLE(RESULT_SCAN(LAST_QUERY_ID())) WHERE LOWER(\"kind\") = 'table'"""
                ).collect()
            )

        if not tables.empty:
            table_list = ["Choose..."] + tables["name"].values.tolist()
            sb_table = st.selectbox(
                f"Select Table:",
                table_list,
    ...
    if st.session_state.current_step == 3:
        ...
        ib_table_created = render_create_iceberg_table(
            st.session_state.ev_name, st.session_state.table
        )

        # write query conversion to table
        if ib_table_created:
            session.sql(
                f"""INSERT INTO "CONVERT2ICEBERG_<MY_ROLE>".LOGGING.CONVERSION_LOG(id
                                                                        ,conversion_type
                                                                        ,original_tables
                                                                        ,converted_tables
                                                                        ,timestamp)
    ...
    disable_next_button = (
        True
        if st.session_state.current_step == 3
        or (st.session_state.current_step == 1 and st.session_state.disable_step_2)
        or (st.session_state.current_step == 2 and st.session_state.disable_step_3)
        else False
    )

    form_footer_cols = st.columns([14, 1.875, 1.875])

    form_footer_cols[0].button(
        "Home", type="secondary", on_click=set_page, args=["home"]
    )
...
def render_choose_query_wizard_view():
    ...
    st.markdown(
        "<h2 style='text-align: center; color: black;'>Convert Table(s) from Query</h2>",
        unsafe_allow_html=True,
    )

    ###### Top Navigation ######
    btn_choose_query_id_type = (
        "primary" if st.session_state.current_step == 1 else "secondary"
    )
    btn_query_stats_type = (
        "primary" if st.session_state.current_step == 2 else "secondary"
    )
    btn_convert_table_type = (
        "primary" if st.session_state.current_step == 3 else "secondary"
    )
    btn_view_qry_type = "primary" if st.session_state.current_step == 4 else "secondary"

    st.write("")
    st.write("")
    step_cols = st.columns([0.5, 0.55, 0.55, 0.55, 0.55, 0.5])
    step_cols[1].button(
        "STEP 1",
        on_click=set_form_step,
        args=["Jump", 1],
        type=btn_choose_query_id_type,
        disabled=False,
    )
    ...
    step_cols[4].button(
        ...
        on_click=set_form_step,
        args=["Jump", 4],
        type=btn_view_qry_type,
        disabled=st.session_state.disable_step_4,
    )
    ...
    if st.session_state.current_step == 1:
        ...
        sb_query_option = st.selectbox(
            "Either provide a Query ID or select a query from QUERY_HISTORY",
            query_option_list,
        ...
        if sb_query_option == "Enter Query ID":
            txt_enter_query_id = st.text_input(
                "Enter a Query ID:",
                st.session_state.enter_query_id,
            ...
            if txt_enter_query_id:
                df_check_query_id = pd.DataFrame(
                    session.sql(
                        f"""SELECT 
                                                                    QUERY_ID
                                                                    ,END_TIME 
                ...
                if not df_check_query_id.empty:
                    ...
                    check_query_end_time = df_check_query_id.iloc[0, 1].replace(
                    ...
                    if query_time_diff.days > 14:
                        st.warning(
                            f"The query belonging to the Query ID entered was executed **{query_time_diff.days}** days ago, which is greater than the max 14 days allowed. Please enter another Query ID.",
                            icon="⚠️",
                        )
                    else:
                        st.session_state.selected_query_id = (
                    ...
                    st.warning(
                        f"Invalid Query ID. Ensure that the query was successfully executed by this role. If so, please check back later or enter another Query ID.",
                        icon="⚠️",
                    )
        ...
        if sb_query_option == "Choose Query":
            ...
            sb_query_history_range = st.selectbox(
                "Select Query History Date Range:",
                query_history_range_list,
                ...
                args=(
                    "query",
                    "sb_query_history_range",
                    "sb_query_history_range_idx",
                    query_history_range_list,
                ),
            )
            ...
            if sb_query_history_range != "Choose a Date Range":
                with st.spinner("Fetching..."):
                    st.session_state.df_query_history_range = u.query_cache(
                        session,
                        f"""SELECT
                                                                                           --query details
                                                                                            QUERY_ID
        ...
        if (
            sb_query_option == "Enter Query ID"
            and st.session_state.selected_query_id
            and st.session_state.selected_query_id == st.session_state.enter_query_id
        ) or (
            sb_query_option == "Choose Query"
            and st.session_state.selected_query_id
            and not st.session_state["df_query_history_range"].empty
        ):
            st.write("")
    ...
    if st.session_state.current_step == 2:
        ...
        if st.session_state.selected_query_id:
            ...
            if not df_query_history.empty:
                ...
                st.write("#")
                st.error(
                    f"Query ID: **{st.session_state.selected_query_id}** does not exist, not yet available in ACCOUNT_USAGE or this role does not have privileges to the query. Please return to STEP 1 and select another query.",
                    icon="🚨",
                )
    ...
    if st.session_state.current_step == 3:
        ...
        if not st.session_state.query_op_stats_check:
            ...
            df_query_operator_stats = u.query_cache(
                session,
                f"""SELECT
        ...
        if st.session_state.convert_tbl_select_list:
            st.write(
                f"The following tables from the Query ID **{st.session_state.selected_query_id}** can be converted to Iceberg tables:"
            )
            ...
            st.write("")
            sb_select_convert_tbl = st.selectbox(
                "Select Table:",
                sb_select_convert_list,
                ...
                args=(
                    "query",
                    "sb_select_convert_tbl",
                    "sb_select_convert_idx",
                    sb_select_convert_list,
                ),
            )
            ...
            if sb_select_convert_tbl != "Choose...":
                ...
                if st.session_state.flag_render_select_ev:
                    ...
                    ib_table_created = render_create_iceberg_table(
                        ev_name, sb_select_convert_tbl
                    )

                    if ib_table_created:
                        # replace original table in query with iceberg table
                        if (
                            f"{sb_select_convert_tbl}_IT"
                            not in st.session_state.converted_query
                        ):
                            st.session_state.converted_query = (
                                st.session_state.converted_query.replace(
                                    sb_select_convert_tbl, f"{sb_select_convert_tbl}_IT"
                                )
                            )
                        ...
                        if (
                            f"{sb_select_convert_tbl}_IT"
                            not in st.session_state.converted_tbl_list
                        ):
                            st.session_state.converted_tbl_list.append(
                                f"{sb_select_convert_tbl}_IT"
                            )
            ...
            st.error(
                f"There are no eligible tables from Query ID: **{st.session_state.selected_query_id}** Please return to STEP 1 and select another query.",
                icon="🚨",
            )
    ...
    if st.session_state.current_step == 4:
        ...
        st.write("")
        st.write(
            "The table(s) in the original query have been replaced with the Iceberg table(s) created in the Step 3. Click **Run Query** below to execute the query."
        )
        st.write("")
        ...
        if query_executed:
            st.write("")
            st.success(
                f"""Query: **{last_query_id}** successfully executed. 🎉 
    ...
    disable_next_button = (
        ...
        or (st.session_state.current_step == 1 and st.session_state.disable_step_2)
        or (st.session_state.current_step == 2 and st.session_state.disable_step_3)
        or (st.session_state.current_step == 3 and st.session_state.disable_step_4)
        else False
    )
...
def render_query_comparison_view():
    st.markdown(
        "<h2 style='text-align: center; color: black;'>Query Comparisons</h2>",
        unsafe_allow_html=True,
    )
    st.write("")
    st.write(
        "The table below contains the Iceberg tables created either by selecting an existing table or query."
    )
    st.write(
        "If queries were used to convert tables and the revised query was executed in this app (via **Step 4** of the Choose Query page), the execution stats of old and new queries can be can be viewed and compared by clicking **Compare** below."
    )
    st.write("#")
    ...
    col1.markdown(
        '<span style="font-size: 14px;">**Date**</span>', unsafe_allow_html=True
    )
    col2.markdown(
        '<span style="font-size: 14px;">**Conversion Type**</span>',
        unsafe_allow_html=True,
    )
    col3.markdown(
        '<span style="font-size: 14px;">**Original Tables**</span>',
        unsafe_allow_html=True,
    )
    col4.markdown(
        '<span style="font-size: 14px;">**Original Query**</span>',
        unsafe_allow_html=True,
    )
    col5.markdown(
        '<span style="font-size: 14px;">**Converted Tables**</span>',
        unsafe_allow_html=True,
    )
    col6.markdown(
        '<span style="font-size: 14px;">**Converted Query**</span>',
        unsafe_allow_html=True,
    )
    col7.markdown(
        '<span style="font-size: 14px;">**Compare**</span>', unsafe_allow_html=True
    )

    df_conversion_log = pd.DataFrame(
        session.sql(
            f"""SELECT
                                                        ID
                                                        ...
                                                        ,CONVERTED_QUERY
                                                        ,CONVERTED_QUERY_ID
                                                    FROM "CONVERT2ICEBERG_<MY_ROLE>".LOGGING.CONVERSION_LOG ORDER BY TIMESTAMP DESC"""
        ).collect()
    )
    ...
    for index, row in df_conversion_log.iterrows():
        ...
        col1.markdown(
            f'<span style="font-size: 12px;">{date}</span>', unsafe_allow_html=True
        )
        col2.markdown(
            f'<span style="font-size: 12px;">{type}</span>', unsafe_allow_html=True
        )
        for tbl in org_tables:
            col3.markdown(
                f'<li style="font-size: 12px;">{tbl}</li>', unsafe_allow_html=True
            )
        col4.markdown(
            f'<span style="font-size: 12px;">{org_query}</span>', unsafe_allow_html=True
        )
        ...
        col6.markdown(
            f'<span style="font-size: 12px;">{converted_query}</span>',
            unsafe_allow_html=True,
        )
...
class home(BasePage):
    ...
    def print_page(self):
        ...
        with col1:
            st.markdown(
                "<h3 style='text-align: center; color: black;'>External Volume</h3>",
                unsafe_allow_html=True,
            )
        ...
        with col2:
            st.markdown(
                "<h3 style='text-align: center; color: black;'>Choose Table</h3>",
                unsafe_allow_html=True,
            )
        ...
        with col1:
            st.markdown(
                "<h3 style='text-align: center; color: black;'>Choose Query</h3>",
                unsafe_allow_html=True,
            )
            ...
            with cq_col2:
                st.button(
                    ...
                    on_click=set_page,
                    args=("choose_query",),
                    key="btn_choose_query",
                )

        with col2:
            st.markdown(
                "<h3 style='text-align: center; color: black;'>Query Comparison</h3>",
                unsafe_allow_html=True,
            )
