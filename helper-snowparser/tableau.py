...
metrics (
    ...
    # def try_create_view(self, name, n=3) -> str|None:
    #     from prompts import DDLCorrectionPrompt
    #     from snowflake.snowpark.exceptions import SnowparkSQLException

    #     attempt = 1
    #     query = self.get_view_ddl(name)['ddl']
    #     while attempt <= 3:
    #         try:
    ...
    #             attempt += 1
    #             logger.error("An error occurred at CREATE SEMANTIC VIEW: %s", e)
    #             logger.info("Entering ddl self-correction")
    #             correction_prompt = DDLCorrectionPrompt(
    #                 ddl = query,
    #                 error = e,)
    #             try:
    #                 result = json.loads(complete(correction_prompt.model,
    #                             correction_prompt.prompt,
    #                             options = correction_prompt.options,
    #                             session = self.session)
    #                 )
    #                 query = result.get('ddl')
    #                 explanation = result.get('explanation')
    #                 logger.info("Correction explanation: %s", explanation)
    #             except json.JSONDecodeError as e:
    #                 logger.error("Error unpacking Cortex response in self-correction: %s", e)
    #                 return "Error unpacking Cortex response in self-correction,"
    #             except Exception as e:
    #                 logger.error("Error in self-correction: %s", e)
    #                 return "Error occurred in self-correction."
    #         except Exception as e:
    #             logger.error("Error in issuing CREATE SEMANTIC VIEW SQL: %s", e)
    #             return "Error in issuing CREATE SEMANTIC VIEW SQL."

    #     return f"The Semantic View was not created as self-correction was unsuccessful after {n} attempts. Please review the DDL and try again."
