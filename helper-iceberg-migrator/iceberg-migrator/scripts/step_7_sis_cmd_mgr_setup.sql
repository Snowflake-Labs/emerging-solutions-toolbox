...
SET SCM_DB = 'SIS_CMD_MGR_'||$T2I_ROLE;
SET SCM_WH = 'SIS_CMD_MGR_'||$T2I_ROLE||'_WH';
...
CREATE OR REPLACE PROCEDURE GENERATE_TASKS(polling_interval_secs NUMBER, task_wh VARCHAR)
RETURNS VARCHAR
LANGUAGE PYTHON
RUNTIME_VERSION = '3.11'
PACKAGES = ('snowflake-snowpark-python')
...
def generate_tasks(session, polling_interval_secs, task_wh):
    try:
...
CALL GENERATE_TASKS($TASK_POLLING_INTERVAL_SECS, $SCM_WH);
...
--UNSET VARS
UNSET(T2I_ROLE, SCM_DB, SCM_WH, TASK_POLLING_INTERVAL_SECS);
