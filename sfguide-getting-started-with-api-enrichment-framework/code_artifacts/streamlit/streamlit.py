...
def sql_to_dataframe(sql: str) -> pd.DataFrame:
    return session.sql(sql).collect(
        statement_params={"QUERY_TAG": json.dumps(QUERY_TAG)}
    )
...
class main_page(Base_Page):
    ...
    def print_page(self):
        ...
        scheduled_jobs = sql_to_pandas(
            "SELECT * FROM DATA.SCHEDULED_TASKS ORDER BY CREATED_DATETIME"
        )

        secret_associations = permissions.get_reference_associations("CONSUMER_SECRET")

        external_access_associations = permissions.get_reference_associations(
            "MY_EXTERNAL_ACCESS"
        )
...
class settings_page(Base_Page):
    ...
    def print_page(self):
        ...
        st.write(
            "Please give the permissions to create a database, and tasks so we can schedule your jobs!"
        )
        if st.button("Grant Permissions"):
            permissions.request_account_privileges(
                ["CREATE DATABASE", "EXECUTE MANAGED TASK"]
            )

        if (
            len(secret_associations) != 0
            and len(external_access_associations) != 0
            and permissions.get_held_account_privileges(
                ["CREATE DATABASE", "EXECUTE MANAGED TASK"]
            )
        ):
...
class table_mapping_page(Base_Page):
    ...
    def print_page(self):
        ...
        reference_associations = permissions.get_reference_associations(
            "enrichment_table"
        )
        tables = {}
        if len(reference_associations) == 0:
            ...
            for table in reference_associations:
                ...
                sqlstring = (
                    f"""show columns in table reference('enrichment_table','{table}')"""
                )
    ...
    def print_sidebar(self):
        with st.sidebar:
            super().print_sidebar()
            st.button(
                "Go to main", key="sidebar button", on_click=set_page, args=("main",)
            )
...
class page_two(Base_Page):
    ...
    def print_page(self):
        ...
        fields = sql_to_dataframe(
            "SELECT VALUE FROM DATA.METADATA WHERE KEY = 'FIELDS'"
        )
        ...
        if len(fields) == 0:
            st.warning(
                "There are no fields, please have the admin configure the fields"
            )
        else:
            fields = json.loads(fields[0][0])["fields"]
            for field in fields:
                map_dict[field] = st.selectbox(
                    f"Pick a column to map ***{field}*** to",
                    columns,
                    key="pick_" + field,
                )
    ...
    def print_sidebar(self):
        with st.sidebar:
            super().print_sidebar()
            st.button(
                "Go to main", key="sidebar button 2", on_click=set_page, args=("main",)
            )
